...
Measured with [alibi](https://github.com/BOTIROFF-D/alibi) over merged pull requests written by five coding agents. Generated 2026-09-13T02:44:30.101Z.
...
| reason | count |
| --- | --- |
| the suite could not be started here | 47 |
| suite already failing at the base commit | 30 |
| the suite needs infrastructure this harness does not provide | 29 |
| no test produced a definitive answer against the base source | 19 |
| npm install failed | 12 |
| pip install -e . failed | 11 |
| commits no longer fetchable | 10 |
| pip install -r requirements.txt failed | 9 |
| no package.json | 7 |
| go mod download failed | 5 |
| harness error: no test runner found. Pass --runner, or put a command in alibi.json — see the README. | 1 |
...
| --- | --- | --- |
| tests added and answered | 192 | 100% |
| **went red without the change** | **125** | **65.1%** |
| **passed without the change** | **67** | **34.9%** |

A further 37 added tests could not be answered at all: they never ran against the base source, because the code they import did not exist yet. That is expected for new modules and is evidence in neither direction, so they are outside the denominator rather than inside it on favourable terms.

2 of the 20 measured pull requests (10.0%) added tests of which **not one** would have failed without the change.

Alongside that: 4 existing tests were deleted or switched off in these diffs, and 4 added tests carry an assertion that cannot fail or no assertion at all.

## By language

| language | pull requests | tests answered | passed without the change |
| --- | --- | --- | --- |
| typescript | 8 | 77 | 28 (36.4%) |
| python | 4 | 45 | 32 (71.1%) |
| go | 4 | 38 | 0 (0.0%) |
...
| --- | --- | --- | --- |
| claude | 6 | 82 | 25 (30.5%) |
| devin-ai-integration | 8 | 80 | 38 (47.5%) |
| codegen-sh | 1 | 16 | 0 (0.0%) |
| copilot-swe-agent | 5 | 14 | 4 (28.6%) |
...
| [federicos-svg/fantacalcio-public#109](https://github.com/federicos-svg/fantacalcio-public/pull/109) | lo sfidante entra al sesto e non prima (§2.4 punti 3 e 5) › al quinto NON entra, per quanto domini | `packages/league-gameweek/tests/championChallengerLedger.test.ts:83` |
| [federicos-svg/fantacalcio-public#109](https://github.com/federicos-svg/fantacalcio-public/pull/109) | lo sfidante entra al sesto e non prima (§2.4 punti 3 e 5) › al sesto entra, e i sei numeri del cambio sono quelli di (a), (b), (c) | `packages/league-gameweek/tests/championChallengerLedger.test.ts:101` |
...
| [amarazzi/argentina-news-digest#9](https://github.com/amarazzi/argentina-news-digest/pull/9) | test_avisa_cuando_el_dia_salio_flojo | `tests/test_alert.py:19` |
| [amarazzi/argentina-news-digest#9](https://github.com/amarazzi/argentina-news-digest/pull/9) | test_no_avisa_cuando_el_dia_estuvo_normal | `tests/test_alert.py:29` |
